-- Extend workout_type with 8 more values (paos, rope, prepa, cycling, rowing,
-- circuit, mobility, yoga). SQLite can't alter a CHECK in place, so the table
-- is rebuilt. No foreign keys reference workout, so a plain copy is safe.
...
CREATE TABLE workout_new (
	...
	workout_type TEXT NOT NULL CHECK (workout_type IN (
		...
		'paos', 'rope', 'prepa', 'cycling', 'rowing', 'circuit', 'mobility', 'yoga'
	)),
